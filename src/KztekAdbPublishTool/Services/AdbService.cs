...
/// <summary>
/// Wraps platform-tools\adb.exe. adb không có broadcast API tiện dụng cho desktop app
/// nên phát hiện thiết bị dùng polling "adb devices -l" định kỳ (xem MainForm).
/// </summary>
public sealed class AdbService
{
    ...
    public async Task<AdbCommandResult> RunAsync(string arguments, int timeoutMs = 30000, CancellationToken ct = default)
    {
        if (!File.Exists(_adbPath))
        {
            ...
            {
                ExitCode = -1,
                StdErr = $"Không tìm thấy adb.exe tại: {_adbPath}"
            };
        }
    ...
    /// không phụ thuộc vào việc user có nhập đúng package cần theo dõi hay không.
    /// </summary>
    ...
    /// KHÔNG dùng "monkey -p pkg -c LAUNCHER" — monkey chỉ gửi 1 sự kiện ngẫu nhiên khớp category, exit code
    /// không nhất quán giữa các bản ROM, và trên nhiều thiết bị thực tế KHÔNG mở được app dù log trông như thành công.
    /// </summary>
    ...
    public async Task<string?> GetPackageVersionAsync(string serial, string packageName, CancellationToken ct = default)
    {
        ...
        foreach (var line in result.StdOut.Split('\n'))
        {
            ...
            if (trimmed.StartsWith("versionName=", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // không phải lỗi đọc, coi như "không có version" thay vì hiển thị chuỗi "null" gây hiểu lầm.
