...
/// — không cần aapt/aapt2, chỉ dùng System.IO.Compression để mở .apk như .zip rồi tự parse AXML.
/// Đăng ký là Singleton trong DI (stateless, thread-safe).
/// </summary>
public sealed class ApkManifestReader
{
    private const int ChunkStringPool = 0x0001;
    private const int ChunkXmlStartElement = 0x0102;

    public string? TryGetPackageName(string apkPath)
    {
        try
        {
        ...
        catch
        {
            ...
            // để user tự nhập package thủ công.
            return null;
        }
    }
    ...
    private static string? ParsePackageName(byte[] data)
    {
        ...
        while (reader.BaseStream.Position <= reader.BaseStream.Length - 8)
        {
            ...
            else if (chunkType == ChunkXmlStartElement)
            {
                ...
                // Attribute "package" luôn nằm trên thẻ <manifest> gốc
    ...
    {
        ReadUtf8Length(reader); // số ký tự — không cần dùng
