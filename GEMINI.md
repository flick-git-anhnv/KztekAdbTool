# GEMINI.md — AI Agent Creator Workspace
## Hướng dẫn hành vi BẮT BUỘC cho Gemini Agent

> **CẢNH BÁO:** Mọi chỉ dẫn trong tài liệu này là BẮT BUỘC. Gemini Agent KHÔNG được tự ý bỏ qua, rút gọn, hoặc thay đổi thứ tự agent. Vi phạm bất kỳ quy tắc nào đều coi là lỗi quy trình.
...
Gemini Agent hoạt động như **Dispatcher** — không phải như một AI tự do.
...
> **Skill `/ship`** (`.gemini/commands/ship.md`): gate GO/NO-GO tùy chọn trước deploy cho WF-HOTFIX/WF-FASTTRACK/WF-BUGFIX — fan-out song song Tech Lead + Security Audit + QA, không thay thế chain đầy đủ của WF-FEATURE.
...
- Documentation Writer PHẢI đọc `.gemini/commands/kztek-brand-info.md` trước khi tạo bất kỳ file nào.
...
**Nguyên tắc cứng (xem `.gemini/agents/github-repo-researcher.md` chi tiết):**
...
|---|---|
| `GEMINI.md` | §1 org chart, §2 bảng routing, §4 (thêm/sửa workflow nếu agent tham gia luồng), §13.1 bảng model |
| `.gemini/shared/CORE.md` | §2 chain of command, §3 bảng routing, §7 model assignment |
...
| `.gemini/agents/[name].md` | File định nghĩa agent — nguồn sự thật về model/tools/scope |

**Checklist bắt buộc trước khi coi việc thêm agent là DONE:**
```
- [ ] Agent có xuất hiện trong org chart (GEMINI.md §1 + CORE.md §2 + RULES.md §1)?
- [ ] Agent có dòng trong bảng routing (GEMINI.md §2 + CORE.md §3), nêu rõ điều kiện kích hoạt?
- [ ] Model của agent có trong bảng §13.1 GEMINI.md + §7 CORE.md?
- [ ] Nếu agent chèn có điều kiện vào workflow có sẵn (VD: chỉ khi đổi UI) → đã thêm bước điều kiện vào §4 GEMINI.md + note tương ứng RULES.md §3?
...
> Chi tiết artifact bắt buộc của từng agent: xem file tương ứng trong `.gemini/agents/[agent-name].md`
...
> Nội dung đầy đủ (test data naming, CRUD coverage, chu trình SETUP/EXECUTE/CLEANUP, các lệnh cấm): xem `.gemini/agents/qa-engineer.md`
...
| Agent | Model | Lý do |
|---|---|---|
| **CTO** | `gemini-3.6-pro` | Quyết định kiến trúc chiến lược, trade-off phức tạp, rủi ro cao |
| **Tech Lead** | `gemini-3.6-pro` | Thiết kế kỹ thuật phức tạp, API contract, review kiến trúc sâu |
| **Product Manager** | `gemini-3.6-flash` | Viết PRD, phân tích yêu cầu, quyết định scope — trung bình |
| **Business Analyst** | `gemini-3.6-flash` | User story, AC dạng Given/When/Then, phân tích nghiệp vụ |
| **Engineering Manager** | `gemini-3.6-flash` | Phân bổ resource, quyết định ưu tiên, quản lý team |
| **Senior Developer** | `gemini-3.6-flash` | Code phức tạp, code review, mentor — cần reasoning tốt |
| **QA Lead** | `gemini-3.6-flash` | Test strategy, risk assessment, sign-off decisions |
| **DevOps Lead** | `gemini-3.6-flash` | Kiến trúc infra, incident management, approve deploy |
| **UI/UX Designer** | `gemini-3.6-flash` | Design spec, wireframe, UX reasoning |
| **Junior Developer** | `gemini-3.6-flash` | Code CRUD + UI, cần reasoning đủ để viết test và hiểu pattern |
| **QA Engineer** | `gemini-3.6-flash` | Phân tích bug, viết test case phức tạp, verify fix — cần reasoning tốt |
| **DevOps Engineer** | `gemini-3.6-flash` | Viết IaC, pipeline, debug deployment — cần hiểu context rộng |
...
### 13.1b Model routing 3 tầng theo loại task (bổ sung — lấy cảm hứng từ Ruflo/Gemini Flow)
...
|---|---|---|
| Tầng 1 — Cao | `gemini-3.6-pro` | Quyết định kiến trúc, trade-off rủi ro cao, lập plan migrate — giữ nguyên theo §13.1 |
...
- [`.gemini/agents/`](.gemini/agents/) — Định nghĩa chi tiết từng agent
...
**Template:** xem `.gemini/templates/CODE-GRAPH-template.md`
...
Đề xuất tái sử dụng:
  ...
  ├─ Model gợi ý: [gemini-3.6-flash / gemini-3.6-pro]
  └─ File đề xuất: .gemini/agents/[name].md / .gemini/commands/[name].md
...
| Tiêu chí | Agent (`.gemini/agents/*.md`) | Skill (`.gemini/commands/*.md`) |
|---|---|---|
...
> TRƯỚC khi tạo file định nghĩa agent/skill mới, PHẢI tạo file eval tương ứng tại `.gemini/evals/[name].md` theo template `.gemini/templates/EVAL-template.md`. Sau khi implement, chạy thử các ví dụ trong eval và ghi kết quả pass/fail. Agent/skill chỉ được thêm vào routing table (GEMINI.md §2) khi có ≥ 2/3 Capability Eval pass.

**Thứ tự bắt buộc khi tạo agent/skill mới:**
```
1. Tạo .gemini/evals/[name].md từ EVAL-template.md — điền CE-01, CE-02, CE-03
2. Tạo .gemini/agents/[name].md hoặc .gemini/commands/[name].md
...
**Vị trí lưu:**
- Eval: `.gemini/evals/[name].md` (PHẢI tạo TRƯỚC)
- Agent: `.gemini/agents/[name].md`
- Skill: `.gemini/commands/[name].md`
